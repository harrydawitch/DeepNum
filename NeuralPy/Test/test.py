...
model.add(Dense(128,input_size= X_train.shape[0], initializer= 'he_uniform'))
model.add(BatchNormalization())
model.add(Dropout(0.95))
model.add(ReLU(negative_slope=0.25))
model.add(Dense(64, initializer= 'he_uniform'))
model.add(BatchNormalization())
model.add(Dropout(0.95))
model.add(ReLU(negative_slope=0.25))
...
model.compile(optimizer= Adam(0.002, clip= 3.0), losses= categorical_crossentropy(logits= True), metrics= 'accuracy')
