...
class BatchNormalization:
    ...
    def backward(self, da):
        ...
        m = da.shape[1]
        self.dw = np.sum(da * self.x_normalized, axis= 1, keepdims= True)
        self.db = np.sum(da, axis= 1, keepdims= True)

        dX_hat = da * self.weight
        ...
        prev_da = (dX_hat * (1/np.sqrt(self.var + self.epsilon))) + (dvar * (2*(self.inputs - self.mean) / m)) + (dmean * (1/m))
